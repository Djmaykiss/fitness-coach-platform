...
- `src/app/descubre/page.tsx`: seccion "Descubre" del alumno (rutinas destacadas, categorias por zona cruzadas con la biblioteca de ejercicios, articulos). Consume `discoverService` + `exerciseLibraryService`.
...
- `src/services/discover.service.ts` + `src/repositories/mock/discover.repository.ts` + `src/data/discover.ts`: contenido de "Descubre" (seeds estaticos, patron `Mock*` como la landing).
