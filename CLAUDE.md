...
- `src/components/admin/training-programs.tsx`: builder real de programas de entrenamiento en `/admin` (CRUD de programa + dias + ejercicios + asignar a alumno). Consume `trainingService`.
- `src/components/dashboard/training-program-view.tsx`: programa de entrenamiento asignado del alumno (rutina de hoy, dias, ejercicios y "marcar entrenamiento completado"). Consume `trainingService`.
